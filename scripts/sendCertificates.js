...
async function drawCertificateImage(data) {
  ...
  trigrams.forEach((lines, i) => {
    ...
    lines.forEach((isSolid, j) => {
      ...
      if (isSolid) ctx.strokeRect(-22, y, 44, 5);
        ...
        ctx.strokeRect(-22, y, 20, 5);
        ctx.strokeRect(2, y, 20, 5);
      }
    });
    ctx.restore();
  });
  ...
  // 4. 文字排版 (適配 600x424)
  ctx.fillStyle = '#856d28';
  ctx.font = `bold 45px ${fontAntique}`;
  ...
  ctx.font = `15px serif`;
  ctx.fillText('頒 發 給', centerX, 165);

  ctx.fillStyle = '#222222';
  ctx.font = `bold 62px ${fontAntique}`;
  ctx.fillText(data.name || '參加者', centerX, 230);

  ctx.strokeStyle = 'rgba(133, 109, 40, 0.4)';
  ctx.lineWidth = 1.2;
  ctx.beginPath();
  ctx.moveTo(centerX - 110, 242);
  ctx.lineTo(centerX + 110, 242);
  ctx.stroke();
  ...
  ctx.fillText(t2, sx + w1 + gap, 300);
  ctx.fillStyle = 'rgba(34,34,34,0.8)';
  ctx.font = `20px serif`;
  ctx.fillText(t3, sx + w1 + w2 + gap * 2, 300);
  ...
  ctx.fillText('新港文教', s / 2, s / 2 - 13);
  ctx.fillText('基金會印', s / 2, s / 2 + 13);
  ctx.restore();
...
async function run() {
  ...
  console.log(
    `[自動任務] 正在掃描歷史紀錄，補發 ${todayStart} 以前尚未發送的證書...`
  );
  ...
  try {
    ...
    console.log(
      `[自動任務] 資料庫搜尋結果：找到 ${snapshot.size} 筆潛在合格資料`
    );
    ...
    for (const doc of snapshot.docs) {
      ...
      const playDate = data.pickupTime
        ? data.pickupTime.split(' ')[0]
        : '未知日期';
      ...
      console.log(
        `[處理訂單] ${data.name} (${playDate}) - 共有 ${players.length} 位隊員`
      );
