  ...
  return (
    <section className="admin-section form-card">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
        <h3 className="form-section-title" style={{ margin: 0 }}>目前場次管理</h3>
        <label className="submit-btn" title="匯入舊場次" style={{
          background: '#3498db', padding: '0.5rem 1rem', 
          display: 'flex', alignItems: 'center', gap: '6px',
          borderRadius: '8px', cursor: 'pointer', fontSize: '0.8rem',
          height: '32px', border: 'none', color: 'white'
        }}>
          ...
          匯入舊場次
          <input type="file" accept=".xlsx, .xls" onChange={handleImportSessionsExcel} style={{display: 'none'}} />
        </label>
      </div>
      ...
      <div className="add-session-form">
        ...
        <div className="edit-form-grid">
          ...
          <div className="form-group" style={{gridColumn: '1 / -1'}}>
            <label>場次限定時段 (可多選，不選則代表全時段開放，若有勾選則優先以此為準)</label>
            <div className="time-slot-grid">
              {(newSession.isSpecial ? specialTimeSlots : generalTimeSlots).map((t: string) => (
                <button 
                  key={t} 
                  type="button"
